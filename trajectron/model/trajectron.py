...
from model.safty_checker import safety_checker
...
class Trajectron(object):
    ...
    def train_loss(self, batch, node_type):
        ...
        if type(safe_map) == torch.Tensor:
            safe_map = safe_map.to(self.device)

        # Run forward pass
        model = self.node_models_dict[node_type]
        loss, loss_neg = model.train_loss(inputs=x,
                                          inputs_st=x_st_t,
    ...
    def predict(self,
        ...
        for node_type in self.env.NodeType:
            ...
            batch = get_timesteps_data(env=self.env, scene=scene, t=timesteps, node_type=node_type, state=self.state,
                                       ...
                                       max_ft=min_future_timesteps, hyperparams=self.hyperparams, check_checker=check_checker)
            ...
            (first_history_index,
             ...
             neighbors_edge_value,
             robot_traj_st_t,
             map, safe_map), nodes, timesteps_o = batch
            ...
            if check_checker:
                predictions_patch = model.traj2patch(x, predictions)
                check_result = safety_checker(predictions_patch, safe_map.to(self.device))[0].cpu().detach().numpy()
        ...
        if check_checker:
            return predictions_dict, check_result
        else:
