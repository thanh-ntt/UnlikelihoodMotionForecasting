...
def main():
    ...
    hyperparams['pred_state'].pop('PEDESTRIAN')
    ...
    hyperparams['gamma_crossover'] = args.gamma_crossover_epoch * 3639  # 3639 is the number of batches in each epoch. hard coded for current nuscenes setting.
    hyperparams['unlike_type'] = args.unlike_type
    hyperparams['log_bias'] = args.log_bias
    hyperparams['learning_decay_rate'] = args.learning_decay_rate
    ...
    print('| Unlike Type: %s' % hyperparams['unlike_type'])
    print('-----------------------')
    ...
    if not args.debug:
        ...
        random_id = str(np.random.randint(999)).zfill(3)  # avoid the same path when run multiple experiment at the same time
    ...
    for epoch in range(1, args.train_epochs + 1):
        ...
        for node_type, data_loader in train_data_loader.items():
            ...
            for batch in pbar:
                ...
                train_loss, loss_neg = trajectron.train_loss(batch, node_type)
                pbar.set_description(f"Epoch {epoch}, {node_type} L: {train_loss.item():.2f} L_neg: {loss_neg.item():.2f}")

                gamma = trajectron.node_models_dict[node_type].gamma

                (train_loss + gamma * loss_neg).backward()
