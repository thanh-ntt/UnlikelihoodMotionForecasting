    ...
    for node in node_list:
        ...
        if node.type.name == 'VEHICLE':
            ...
            r_img = rotate(cars[i % len(cars)], node.get(np.array([ts_key]), {'heading': ['°']})[0, 0] * 180 / np.pi,
                           reshape=True)
            oi = OffsetImage(r_img, zoom=0.025, zorder=700, alpha=0.6)
...
def plot_kde(model, scene, ph, save_name, save_path='img'):
    with torch.no_grad():
        ...
        save_name = os.path.join(save_path, '{}_{}_{}_{}.png'.format(scene.name, save_name, fde, kde))
        fig.savefig(save_name, dpi=300, bbox_inches='tight')
        plt.close()
        return error_dict


def plot_one(model, scene, ph, save_name, save_path='img'):
    # if creative is None:
    #     full_dist = True
    # else:
    #     full_dist = False
