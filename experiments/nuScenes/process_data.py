...
from environment import Environment, Scene, Node, GeometricMap, SafeMap, derivative_of
...
def chunk_list(x, n_chunk):
    n = int(math.ceil(len(x) / float(n_chunk)))
    return [x[i:i+n] for i in range(0, len(x), n)]


def process_scene_multiprocess(scenes_chunk, version, scene_blacklist, env, data_class, data_path):
    nusc = NuScenes(version=version, dataroot=data_path, verbose=True)

    scenes = []
    for ns_scene_name in scenes_chunk:
        print('process: ' + ns_scene_name)
        ...
        if scene is not None:
            if data_class == 'train':
                ...
                angles = np.arange(0, 360, 15)  # rotate the scene for data augmentation
                for angle in angles:
...
def augment_scene(scene, angle):
    ...
    scene_aug = Scene(timesteps=scene.timesteps, dt=scene.dt, name=scene.name, non_aug_scene=scene)  # doesn't contain map
...
def augment(scene):  # randomly select a augmented scene and init
    scene_aug = np.random.choice(scene.augmented)
    scene_aug.temporal_scene_graph = scene.temporal_scene_graph
    scene_aug.map = scene.map  # augmented scene map is given here  # why not be rotated?
    return scene_aug
...
def process_scene(ns_scene, env, nusc, data_path):
    ...
    sample = nusc.get('sample', sample_token)  # get the data of the first step
    frame_id = 0
    sample_token_list = list()
    while sample:  # the original version skip the last frame. add last frame back
        sample_token_list.append(sample['token'])
        annotation_tokens = sample['anns']
        for annotation_token in annotation_tokens:  # each annotation_tokens is an agent (exclude ego)
            ...
            elif 'vehicle' in category and 'bicycle' not in category and 'motorcycle' not in category and 'parked' not in attribute:  # ignore pared vehicle
        ...
        sample = nusc.get('sample', sample['next']) if sample['next'] else False
        frame_id += 1
    ...
    y_size = y_max - y_min
    patch_box = (
    x_min + 0.5 * (x_max - x_min), y_min + 0.5 * (y_max - y_min), y_size, x_size)  # center_x, center_y, h, w
    patch_angle = 0  # Default orientation where North is up
    canvas_size = (np.round(3 * y_size).astype(int), np.round(3 * x_size).astype(int))
    homography = np.array([[3., 0., 0.], [0., 3., 0.], [0., 0., 3.]])  # homography is hard-coded
    ...
    lane_direction_mask = get_direction_mask(nusc_map, patch_box, canvas_size)
    lane_direction_mask = np.transpose(lane_direction_mask, [2, 1, 0])
    lane_direction_mask = lane_direction_mask[::-1]

    # different types of agents has different maps
    # PEDESTRIANS
    ...
    # VEHICLES SAFE
    safe_mask = np.concatenate([lane_direction_mask, map_mask[2:3]], axis=0)
    type_map['SAFE'] = SafeMap(data=safe_mask, homography=homography, description='lane_direction, drivable_area')
    ...
    for node_id in pd.unique(data['node_id']):
        ...
        node_df = data[data['node_id'] == node_id]  # select data of one agent

        if node_df['x'].shape[0] < 2:  # skip the agent which appears only in one step
            continue

        if not np.all(np.diff(node_df['frame_id']) == 1):  # make sure the data for the agent is continuous in temporal dim
            # print('Occlusion')
...
def process_data(data_path, version, output_path, val_split):
    ...
    if create_val:
        train_scenes, val_scenes = train_test_split(splits['train' if 'mini' not in version else 'mini_train'],
                                                    test_size=val_split)
        train_scene_names = train_scenes
    ...
    for data_class in ['train', 'val', 'test']:
        if len(ns_scene_names[data_class]) == 0:
            continue
        ...
        if n_process > 1:
            ...
            scenes_chunk_list = chunk_list(ns_scene_names[data_class], n_process)
            results = []
            for i, chunk in enumerate(scenes_chunk_list):
                result = p.apply_async(process_scene_multiprocess, args=(chunk, version, scene_blacklist, env, data_class, data_path))
                results.append(result)
            p.close()
            p.join()
            print('get result')
            for result in results:
                scenes.extend(result.get())

            print(f'Processed {len(scenes):.2f} scenes')

        else:
            scenes = process_scene_multiprocess(ns_scene_names[data_class], version, scene_blacklist, env, data_class, data_path)
